...
def generate_guaranteed_tsptw_test_cases():
    ...
    all_N = [5, 10] + list(range(50, 2001, 50))

    for idx, N in enumerate(all_N):
        grid_size = 1000 if N <= 1000 else 10000
        filename = os.path.join(output_dir, f"n{N}.inp")
        ...
        for next_node in hidden_path:
            ...
            # MODIFIED: Massively increased wait times to create early flexibility
            wait_time = random.randint(0, grid_size // 5) 
            e_i = arrival_time + wait_time
            d_i = random.randint(5, 30)
            finish_time = e_i + d_i

            # MODIFIED: Scaled the slack to the grid size. 
            # This allows the vehicle to take massive detours (visit other nodes) 
            # before the deadline actually hits.
            slack = random.randint(grid_size // 2, grid_size * 2)
        ...
        # MODIFIED: Add the cost of returning from the final node to the depot (0)
        total_travel_cost += t[current_node][0]

        # 3. Ghi dữ liệu của bài toán vào file .inp
